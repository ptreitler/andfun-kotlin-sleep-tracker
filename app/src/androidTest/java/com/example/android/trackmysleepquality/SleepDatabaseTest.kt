...
class SleepDatabaseTest {
    ...
    fun insertAndUpdateNight() {
        val night = SleepNight(42)
        sleepDao.insert(night)

        val updatedNight = SleepNight(nightId = 42, sleepQuality = 2)

        sleepDao.update(updatedNight)

        assertEquals(2, sleepDao.getTonight()?.sleepQuality)
    }

    @Test
    fun updateNightThatDoesNotExist() {
        sleepDao.update(SleepNight())
